...
import { useFetchNftMarket } from "@/lib/graphql/request/fetchNft_api";
...
const Card = (cardConfig: CardConfig, onDetailClick: () => void) => {
  return (
    <div
      key={cardConfig.id}
    ...
    >
      ...
      <div className="p-5 flex flex-col justify-center space-y-3">
        ...
        <button
          ...
          className="bg-gradient-to-r from-pink-500 to-purple-500 text-white font-bold py-2 px-4 rounded hover:from-purple-500 hover:to-pink-500 transition duration-300"
        >
          Detail
        </button>
      </div>
...
function NftSection({ title }: { title: string }) {
  const { nftMarketData, error } = useFetchNftMarket();
